...
func TestNewFrameStream(t *testing.T) {
	frame := NewFrameStream()
...
func TestEncode(t *testing.T) {
	...
	err := frame.Encode(rw, []byte("hello"))

	if err != nil {
		t.Errorf("want nil, actual %s", err.Error())
	}
	var len int32
	e := binary.Read(rw, binary.BigEndian, &len)
	if e != nil {
		t.Errorf("want nil, actual %s", e.Error())
	}
	if len != 9 {
		t.Errorf("want 9, actual %d", len)
	}
	left := rw.Bytes()
